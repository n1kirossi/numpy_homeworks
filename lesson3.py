...
# Pandas - расширение NumPy (структурированные массивы). Строки и столбцы индексируются метками, а не только числовыми
# значениями
...
# Index - способ организации ссылки на данные объектов Series и DF. Index - неизменяем, упорядочен, является
# мультмножеством (могут быть повторяющиеся значения)
...
# data = pd.Series([0.25, 0.5, 0.75, 1], index=[1, 3, 3, 15])
#
# print(data[1])
#
# print(data.loc[1])
